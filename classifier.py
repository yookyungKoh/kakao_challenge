...
class Classifier():
    ...
    def train(self, data_root, out_dir):
        ...
        model = TextOnly(opt).to(device)
        criterion = nn.BCEWithLogitsLoss() # float tensor for target
        ...
        for epoch in range(opt.num_epochs):
            ...
            for i, (inputs, targets) in enumerate(train_loader):
                inputs = inputs.type(torch.FloatTensor).to(device) #to device setting !
                ...
                if (i+1)%10000 == 0:
                    print('Epoch [%d/%d] | Step [%d/%d] | Loss: %.4f'
                            %(epoch+1, opt.num_epochs, i+1, len(train_loader), train_loss/(i+1)))

            model.eval()
            val_loss = 0.
            with torch.no_grad():
                for j, (inputs, targets) in enumerate(valid_loader):
                    ...
                    loss = criterion(out, targets)
                    val_loss += loss.item()
            print('Epoch [%d/%d] | Val Loss: %.4f' %(epoch+1, opt.num_epochs, val_loss/((j+1)))
